...
#include "config/spescoin-config.h"
#endif
...
//    strUsage += HelpMessageOpt("-gmnotify=<cmd>", _("Execute command when a gm message is received (%s in cmd is replaced by message)"));
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "spescoin.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "spescoind.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 32822, 42822));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in spescoin/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in spescoin/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, gm ,bench, coindb, db, lock, rand, rpc, selectcoins, mempool, net, proxy, spescoin, (obfuscation, swiftx, masternode, mnpayments)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in spescoin/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all spescoin specific functionality (Masternodes, Obfuscation, SwiftX) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:32822"));
    ...
    strUsage += HelpMessageOpt("-anonymizespescoinamount=<n>", strprintf(_("Keep N spescoin anonymized (default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 32821, 42821));
    ...
    // Run node without keeping private key in wallet db.
    strUsage += HelpMessageOpt("-storekey", _("Store keys in wallet db file, default: true."));
...
std::string LicenseInfo()
{
    return FormatParagraph(_("Copyright (C) 2009-2014 The Bitcoin Core Developers")) + "\n" +
           ...
           "\n" +
           FormatParagraph(_("Copyright (C) 2018-2019 The SpesCoin Core Developers")) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that spescoin is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    fGM = GetBoolArg("-gmessages", DEFAULT_GM);
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. SPES is shutting down."));
...
#endif
    // Make sure only a single spescoin process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. SPES is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("spescoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (mapArgs.count("-gmkey")) // gm priv key
    {
        GMPrivKey = GetArg("-gmkey", "");
        if (!GameMaster.Sign(GMPrivKey)) {
            return InitError(_("Unable to sign gm message, wrong key?"));
        }
    }
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of SPES") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart SPES to complete") << "\n";
    ...
    nAnonymizePhcAmount = GetArg("-anonymizespescoinamount", 0);
    ...
    LogPrintf("Anonymize spescoin Amount %d\n", nAnonymizePhcAmount);
    ...
    /* Denominations
       ...
       For example:
       1spescoin+1000 == (.1spescoin+100)*10
       10spescoin+10000 == (1spescoin+1000)*10
    */

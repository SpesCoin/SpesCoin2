...
QFontDatabase::addApplicationFont(":/fonts/spescoin_font");

QString windowTitle = tr("SPES") + " - ";
#ifdef ENABLE_WALLET
...
void BitcoinGUI::createActions(const NetworkStyle* networkStyle)
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a SPES address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and SPES: URIs)"));
...
*/
    ...
    aboutAction = new QAction(networkStyle->getAppIcon(), tr("&About SPES wallet"), this);
    aboutAction->setStatusTip(tr("Show information about SPES wallet"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for SPES wallet"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your SPES addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(GUIUtil::getThemeImage(":/icons/transaction_0")), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified SPES addresses"));
    ...
    openAction->setStatusTip(tr("Open a SPES: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the SPES help message to get a list with possible SPES command-line options"));
...
void BitcoinGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("SPES client") + " " + networkStyle->getTitleAddText();
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to SPES network", "", count));
}
...
{
    QString strTitle = tr("SPES"); // default title
...
static bool GetAndCompareVersion(BitcoinGUI* gui,  QString &version, QString &downloadUrl) {
    ...
    bool fHaveNew = false;
    bool fRet = false;
    QString url("https://binaries.spescoin.network/downloads/spescoinwallet/index.json");
    QString body;
    ...
    if(fRet) fRet = GetQtJsonObject(object, "spescoinwallet", walletObj);
    if(fRet) {
        ...
        #endif
        if(baseUrl.right(1) == "/"){
            downloadUrl = baseUrl + loaderStr + "?filename=spescoin-" + version + platformStr + suffixStr;
        }
        else{
            downloadUrl = baseUrl +  "/" + loaderStr + "?filename=spescoin-" + version + platformStr + suffixStr;
        }

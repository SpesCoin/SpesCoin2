...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::SPES)
    {
    }
...
//    ui->label_spescoin4->setVisible(showImmature || showWatchOnlyImmature);
...
//        model->getWatchBalance(), model->getWatchUnconfirmedBalance(), model->getWatchImmatureBalance());
        ...
        connect(ui->blabel_spescoin, SIGNAL(clicked()), this, SLOT(openMyAddresses()));

        emit model->makeBalance();
    }

    // update the display unit, to not use the default ("spescoin")
    updateDisplayUnit();
}
...
void OverviewPage::updateMasternodeInfo()
{
  ...
  if (masternodeSync.IsBlockchainSynced() && masternodeSync.IsSynced())
  {
   ...
   std::vector<CMasternode> vMasternodes = mnodeman.GetFullMasternodeMap();
    ...
    BlockReward -= BlockReward * GetSporkValue(SPORK_11_DEVFUND_FEE) / 100;
  ...
  if (CurrentBlock >= 0) {
      ui->label_lcolat->setText("1000 SPES");
      ui->label_mcolat->setText("25000 SPES");
      ui->label_fcolat->setText("80000 SPES");
  }
...
void OverviewPage::updateBlockChainInfo()
{
    if (masternodeSync.IsBlockchainSynced())
    {
        ...
        double BlockReward = GetBlockValue(CurrentBlock);
        double BlockRewardspescoin =  static_cast<double>(BlockReward/COIN);
        ...
        ui->label_CurrentBlockReward_value->setText(QString::number(BlockRewardspescoin, 'f', 1).append(" | ") + QString::number(GetSporkValue(SPORK_11_DEVFUND_FEE)).append("%"));

        ui->label_Supply_value->setText(QString::number(chainActive.Tip()->nMoneySupply / COIN).append(" SPES"));
...
void OverviewPage::on_pushButton_Website_clicked() {
    QDesktopServices::openUrl(QUrl("https://myspes.org", QUrl::TolerantMode));
}
void OverviewPage::on_pushButton_Discord_clicked() {
    QDesktopServices::openUrl(QUrl("https://discord.gg/YxcNwut", QUrl::TolerantMode));
}
void OverviewPage::on_pushButton_Telegram_clicked() {
    QDesktopServices::openUrl(QUrl("https://t.me/spestoken", QUrl::TolerantMode));
}
...
void OverviewPage::on_pushButton_Explorer_clicked() {
    QDesktopServices::openUrl(QUrl("http://explorer.myspes.org/", QUrl::TolerantMode));
}

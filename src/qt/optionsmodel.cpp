...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::SPES);
...
// Wallet
    ...
    if (settings.contains("nAnonymizePhcAmount"))
        SoftSetArg("-anonymizespescoinamount", settings.value("nAnonymizePhcAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in spescoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case CheckVersion:
            return settings.value("fCheckVersion", true);
        case MinimizeToTray:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case StartAtStartup:
            ...
            break;
        case CheckVersion:
            if (settings.value("fCheckVersion") != value) {
                settings.setValue("fCheckVersion", value.toBool());
            }
            break;
        case MinimizeToTray:

...
#if defined(NDEBUG)
#error "spescoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uspescoin) are considered zero fee (for relaying and mining)
...
/**
 * Maintain the history of invalid blocks from peers to trigger a block spam filter,
* this is a protection for anti "Fake Stake" attack.
 */
class CNodeBlocks
{
public:
...
bool CheckTransaction(const CTransaction& tx, CValidationState& state)
{
    ...
    // Size limits
    if (::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION) > SPES_MAX_BLOCK_SIZE)
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
...

    {
        ...
        if (nSigOps > SPES_MAX_TX_SIGOPS)
            return state.DoS(0,
                error("AcceptToMemoryPool : too many sigops %s, %d > %d",
                    hash.ToString(), nSigOps, SPES_MAX_TX_SIGOPS),
...
CAmount GetBlockValue(int nHeight)
{
    ...
    if (Params().NetworkID() == CBaseChainParams::TESTNET){
        if ( nHeight == 0 ) {
            nSubsidy = 50000 * COIN; // premine testnet
        } else if (nHeight < 200) {
            nSubsidy = 5 * COIN;
        } else if (nHeight >= 200 && nHeight < 5000) {
            nSubsidy = 10 * COIN;
        ...
        if ( nHeight == 0 ) {
            nSubsidy = 20000 * COIN; // premine for swap
        } else if (nHeight < 500) {
            nSubsidy = 10 * COIN;
...
//        return 0;
    ...
    if (nProtocol >= CONSENSUS_FORK_PROTO) {
        switch(mnlevel) {
            ...
            case 2: return blockValue * 0.30;
            case 3: return blockValue * 0.45;
        }
    } else {
        switch(mnlevel) {
            case 1: return blockValue * 0.6;
            case 2: return blockValue * 0.2;
            case 3: return blockValue * 0.1;
        }
    }
...
{
    RenameThread("spescoin-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (nSigOps > SPES_MAX_BLOCK_SIGOPS)
...
bool GetTxValueIn(const CTransaction& tx, CAmount &amount)
{
    CCoinsViewCache view(pcoinsTip);

    if (view.HaveInputs(tx)){
        amount = view.GetValueIn(tx);
        return true;
    }
    else{
        return false;
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    // Size limits
    if (block.vtx.empty() || block.vtx.size() > SPES_MAX_BLOCK_SIZE || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION) > SPES_MAX_BLOCK_SIZE)
    ...
    if (pindexPrev != NULL) {
        ...
        if (nHeight != 0 && !IsInitialBlockDownload()) {
            if (!IsBlockPayeeValid(block, nHeight)) {
                // Payees of all tiers cannot be found, then try to resync master node from committee.
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    //Before save block into FS, check for invalid block in case of "Fake Stake" attack.
    if (block.IsProofOfStake()) {
        LOCK(cs_main);
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
...

    {
        ...
        // Track invalid blocks, ban peer if they exceed threshold.
        if (!ret) {
            // Check spamming
...
int nLoaded = 0;
    try {
        ...
        CBufferedFile blkdat(fileIn, 2 * SPES_MAX_BLOCK_SIZE, SPES_MAX_BLOCK_SIZE + 8, SER_DISK, CLIENT_VERSION);
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // read size
                blkdat >> nSize;
                if (nSize < 80 || nSize > SPES_MAX_BLOCK_SIZE)
                    continue;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    // getaddr message mitigates the attack.
        ...
        ProcessGM(pfrom, strCommand, vRecv);

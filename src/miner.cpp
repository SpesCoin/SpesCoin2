...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // Limit to betweeen 1K and SPES_MAX_BLOCK_SIZE-1K for sanity:
    nBlockMaxSize = std::max((unsigned int)1000, std::min((unsigned int)(SPES_MAX_BLOCK_SIZE - 1000), nBlockMaxSize));
...

    {
        ...
        while (!vecPriority.empty()) {
            ...
            if (nBlockSigOps + nTxSigOps >= SPES_MAX_BLOCK_SIGOPS)
                continue;
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("spescoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("spescoinMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("spescoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("spescoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            while (vNodes.size() <= 3 || !masternodeSync.IsSynced() || pwallet->IsLocked() || !fMintableCoins || (pwallet->GetBalance() > 0 && nReserveBalance >= pwallet->GetBalance())) {

                LogPrintf("BitcoinMiner: Peer node size: %d.\n", vNodes.size());

                // wallet should be unlocked already.
                if (pwallet->IsLocked()) {
                    LogPrintf("BitcoinMiner: wallet is locked yet.\n");
                }
                // master node sync works fine here.
                if (!masternodeSync.IsSynced()) {
                    LogPrintf("BitcoinMiner: master node not synced yet.\n");
                }
                // debug if MintableCoins works.
                if (!fMintableCoins) {
                    LogPrintf("BitcoinMiner: Not mintable coin yet. You just need to wait.\n");
                }

                // debug if reserveBalance is sufficient.
                if (nReserveBalance >= pwallet->GetBalance()) {
                    LogPrintf("BitcoinMiner: Not sufficient balance in wallet then reserved balance: %ld.\n", nReserveBalance);
                }
        ...
        //LogPrintf("Running spescoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(), ::GetSerializeSize(*pblock, SER_NETWORK, PROTOCOL_VERSION));
